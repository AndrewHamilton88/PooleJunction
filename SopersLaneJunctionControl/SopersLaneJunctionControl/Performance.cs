...
namespace ParamincsSNMPcontrol
{
    class Performance
    {
        ...
        public double DelayFunctionOtherStagesVer3(int CurrentStage, double TimeActive, List<double[]> RoadState, double CurrentLowestDelay, List<int[]> PhaseList)  //RoadState[0] = Stage1, RoadState[1] = Stage2, RoadState[2] = Stage3, Roadstate[3] = Stage4
        {
            double DelayOnOtherStages = 0; //This function does not include any delay caused to the current stage, and Version 3 calculates average road delay...
            ...
            for (int Phase = 1; Phase < NumberOfPhases + 1; Phase++)
            {
                if (DelayOnOtherStages <= CurrentLowestDelay)
                {
                    if (!ActivePhases.Contains(Phase))
                    {
                        double NumberOfVehiclesAffected = RoadState[Phase - 1][0] + RoadState[Phase - 1][1] * TimeActive;
                        if (NumberOfVehiclesAffected > 0)
                        {
                            DelayOnOtherStages += (TimeActive * (RoadState[Phase - 1][0] + ((TimeActive + 1) * (RoadState[Phase - 1][1]) / 2))) / NumberOfVehiclesAffected;
                        }
                    }
                }
                else
            ...
            return DelayOnOtherStages;      //This is now a sum of the average delay on each of the unreleased phases
        }

        /// <summary>
        /// This uses the second set of delay calculations that I carried out in my notebook! This defines delay as the queue length at the end of each second. Considers delay per vehicle on each phase...
        /// </summary>
        /// <param name="CurrentStage"></param>
        /// <param name="TimeActive"></param>
        /// <param name="RoadState"></param>
        /// <param name="PhaseList"></param>
        /// <returns></returns>
        public double DelayFunctionCurrentStageVer3(int CurrentStage, double TimeActive, List<double[]> RoadState, List<int[]> PhaseList)  //RoadState[0] = Stage1, RoadState[1] = Stage2, RoadState[2] = Stage3, Roadstate[3] = Stage4
        {
            double DelayOnActiveStage = 0;
            ...
            if (CurrentStage != FixedVariables.IntergreenStageNumber)
            {
                foreach (int Phase in PhaseList[CurrentStage - 1])
                {
                    ...
                    if (NumberOfVehiclesAffected > 0)
                    {
                        ...
                        if ((RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) <= TimeActive)        // (queue/discharge) <= timeactive
                        {
                            DelayOnActiveStage += ((RoadState[Phase - 1][0] / (2 * RoadState[Phase - 1][2])) * (RoadState[Phase - 1][0] - RoadState[Phase - 1][2])) / NumberOfVehiclesAffected;  //Equation 1 in my notebook; (n/(2*D)) * (n - D)
                            DelayOnActiveStage += (0.5 * (RoadState[Phase - 1][1] * RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) * ((RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) + 1)) / NumberOfVehiclesAffected;  //Equation 2 in my notebook; (An/2D[n/D + 1])
                            if (RoadState[Phase - 1][1] > RoadState[Phase - 1][2])  //If the arrival rate is greater than the discharge rate
                            {
                                DelayOnActiveStage += ((TimeActive * RoadState[Phase - 1][2] - RoadState[Phase - 1][0]) / (2 * RoadState[Phase - 1][2]) * ((2 * RoadState[Phase - 1][1] * RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) + (RoadState[Phase - 1][1] - RoadState[Phase - 1][2]) * (1 + TimeActive - (RoadState[Phase - 1][0] / RoadState[Phase - 1][2])))) / NumberOfVehiclesAffected;    //Equation 4; (tD - n)/(2D) * (2An/D + (A-D)(1+t-n/D))
                            }
                            if (RoadState[Phase - 1][1] == RoadState[Phase - 1][2])     //If the arrival rate is equal to the discharge rate
                            {
                                DelayOnActiveStage += ((RoadState[Phase - 1][1] * RoadState[Phase - 1][0]) / (RoadState[Phase - 1][2] * RoadState[Phase - 1][2]) * (TimeActive * RoadState[Phase - 1][2] - RoadState[Phase - 1][0])) / NumberOfVehiclesAffected;   //Equation 5; An / D^2 * (tD - n)
                            }
                            else            //If the arrival rate is less than the discharge rate
                            {
                                if (((RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) - ((RoadState[Phase - 1][1] * RoadState[Phase - 1][0]) / (RoadState[Phase - 1][2] * (RoadState[Phase - 1][1] - RoadState[Phase - 1][2])))) <= TimeActive)     // n/D - (An/(D(A-D))) <= t    This represents if the 'arrival queue' can be dissipated within the timeframe (the arrival queue is the queue built up while the initial queue is dissipated)
                                {
                                    DelayOnActiveStage += ((-1 * RoadState[Phase - 1][1] * RoadState[Phase - 1][0]) / (2 * RoadState[Phase - 1][2] * (RoadState[Phase - 1][1] - RoadState[Phase - 1][2])) * ((RoadState[Phase - 1][1] * RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) + (RoadState[Phase - 1][1] - RoadState[Phase - 1][2]))) / NumberOfVehiclesAffected;    // Equation 3;   -An/(2D(A-D) * (An/D + (A-D))
                                }
                                else
                                {
                                    DelayOnActiveStage += ((TimeActive * RoadState[Phase - 1][2] - RoadState[Phase - 1][0]) / (2 * RoadState[Phase - 1][2]) * ((2 * RoadState[Phase - 1][1] * RoadState[Phase - 1][0] / RoadState[Phase - 1][2]) + (RoadState[Phase - 1][1] - RoadState[Phase - 1][2]) * (1 + TimeActive - (RoadState[Phase - 1][0] / RoadState[Phase - 1][2])))) / NumberOfVehiclesAffected;    //Equation 4; (tD - n)/(2D) * (2An/D + (A-D)(1+t-n/D))
                        ...
                        {
                            DelayOnActiveStage += ((TimeActive / 2) * (2 * RoadState[Phase - 1][0] - (RoadState[Phase - 1][2] * (TimeActive + 1)))) / NumberOfVehiclesAffected;    //Equation 6;    t/2 * ( 2n - D*(t+1))
                            DelayOnActiveStage += ((RoadState[Phase - 1][1] * TimeActive / 2) * (1 + TimeActive)) / NumberOfVehiclesAffected;        //Equation 7;    At/2 * (1 + t)
                        }
                    }
                }
            }
            return DelayOnActiveStage;      //This is now a sum of the average delay caused to all the active phases
        }
    }
}
